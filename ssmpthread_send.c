...
inline void ssmpthread_send(uint32_t to, volatile ssmp_msg_t *msg) {
...
inline void ssmpthread_send_no_sync(uint32_t to, volatile ssmp_msg_t *msg) {
#if defined(OPTERON)|defined(local)|defined(XEON) /* --------------------------------------- opteron XEON */
	volatile ssmp_msg_t *tmpm = ssmp_send_buf[to];
	msg->state = BUF_MESSG;
	memcpy((void*) tmpm, (const void*) msg, SSMP_CACHE_LINE_SIZE);
#elif defined(NIAGARA) /* --------------------------------------- niagara */
