...
do
    ...
    then
	oneway=$($run_avg $reps ./main $num_core $num_msgs $num_cores $delay_after | gawk 'BEGIN { min=1e9; }; /through/  { if (min > $4) { min=$4 }; } END {print min}');
    else
    ...
    if [ $do_throughput -eq 1 ];
    then
	roundtrip=$($run_avg $reps ./main_rt $num_core $num_msgs $num_cores | gawk 'BEGIN { min=1e9; }; /through/ { if (min > $4) { min=$4 }; } END {print min}');
    else
